...
// validating webhook can return a field-specific error. A nil override is an
// intentional no-op. component may be nil only for ScopeRoot.
func DefaultTarget(
	...
	// Preserve explicit targets and ignore absent override contexts.
...
// unknown fields, so forward-compatible descendants remain available for the
// provider dry-run to validate against the installed CRD and webhooks.
