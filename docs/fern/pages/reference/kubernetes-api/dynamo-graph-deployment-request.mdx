...
values unless an explicit append modifier is present.
...
string. The target can be the generated `main` container or a sidecar already present in the
generated blueprint. Without `$patch`, specifying `args` retains the standard replacement
behavior.
...
<Note>
  ...
  arguments. In `v1beta1`, container `args` is atomic and replaces the generated list unless
  `$patch.args` is `append`. Use `v1beta1` for new overrides.
</Note>
