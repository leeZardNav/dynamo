...
non-empty string. The target container must already be present in the generated blueprint. The
override helper combines the argument lists and removes `$patch` before returning the final DGD.
Without `$patch`, specifying `args` retains the standard replacement behavior. To override a
container that the Operator adds later, such as a generated frontend `main` container, provide the
complete desired argument list instead.
