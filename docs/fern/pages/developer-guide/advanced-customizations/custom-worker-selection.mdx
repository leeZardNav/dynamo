...
Dynamo's built-in policy selects an eligible affinity target. A custom filter, scorer, or picker can retain or replace it. The frontend rebinds the session only after it dispatches successfully to a different worker.
