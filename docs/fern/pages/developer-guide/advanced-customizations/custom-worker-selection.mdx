...
```mermaid
flowchart LR
    Request["Incoming request"] --> Required{"Explicit target?"}
    Required -->|"Yes"| Validate["Validate and reserve exact target"]
    Required -->|"No"| Eligibility["Dynamo eligibility filters"]
    ...
    Picker --> ValidateRow["Dynamo validates the row"]
    ValidateRow --> Reserve["Accounting and reservation"]
```
...
| `affinity_target()` | Optional preferred worker and data-parallel rank for session affinity |
...
`affinity_target()` returns the session target that the request host resolved before selection. The value remains fixed for every policy callback in one selection attempt.

Session affinity is a preference, not an eligibility constraint. Dynamo keeps all host-eligible workers in the policy candidate table. The affinity target can be absent when the worker is unavailable, overloaded, disallowed, or rejected by a filter.

Dynamo's built-in policy selects an eligible affinity target. A custom filter can reject it, a scorer can change its cost, and a picker can select another eligible worker. The frontend rebinds the session only after it dispatches successfully to a different worker.

An explicit target is different. Dynamo validates and reserves that exact worker without calling custom filters, scorers, or pickers.

The standalone selection service does not own a session-affinity store. Its `select` and `select_and_reserve` requests accept an optional `affinity_target` that an external controller resolved.
