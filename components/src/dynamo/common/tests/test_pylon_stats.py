...
def test_pylon_request_stats_starts_without_claiming_prompt_progress() -> None:
    publisher = Mock()
    PylonRequestStats(publisher, "req-1", "model-a")
...
def test_pylon_request_stats_publishes_cumulative_multi_choice_deltas() -> None:
    publisher = Mock()
    stats = PylonRequestStats(publisher, "req-1", "model-a")
    ...
    assert publisher.publish_stats_event.call_args_list == [
        call("req-1", "model-a", 128, None, False),
        call("req-1", "model-a", None, 2, False),
        call("req-1", "model-a", None, 3, False),
        call("req-1", "model-a", None, 6, False),
    ]
...
def test_pylon_request_stats_terminal_event_is_emitted_once() -> None:
    ...
    assert publisher.publish_stats_event.call_args_list == [
        call("req-1", "model-a", 8, None, False),
        call("req-1", "model-a", None, 3, False),
        call("req-1", "model-a", 8, 3, True),
    ]
